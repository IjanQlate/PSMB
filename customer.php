...
require('db.php');
$query = "SELECT * FROM crecord WHERE id"; 
...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `crecord` WHERE CONCAT(id) LIKE '%".$valueToSearch."%'";
...
<header>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <a class="navbar-brand" href="customer.php">Customer Details</a>
      </li>
    </ul>
	...
	<div id="app">
		...
		<!--display customer-->
				<div class="table-responsive">  
					...
					<tbody>
						<tr class="text-center" v-for="user in crecord.slice().reverse()" :key="user.id" >
							
							<td>{{ user.id }}</td>
                   			<td>
							
							<a v-bind:href="'edit.php?id='+ user.id" target="blank">{{ user.Nama }}</a>
						
							</td>
							<td>{{ user.invois_no }}</td>
                    		<td>{{ user.email }}</td>
                    		<td>{{ user.pNo }}</td>
				...
				<div class="modal-content">
					...
					<div class="modal-body p-4">
						<form action="#" method="post">
							...
							<div class="form-group">
								<input type="text" name="invois_no" class="form-control form-control-lg" placeholder="Invois Number" 
								v-model="newUser.invois_no">
							</div>
							<div class="form-group">
				...
				<div class="modal-content">
					...
					<div class="modal-body p-4">
						<form action="#" method="post">
							...
							<div class="form-group">
								<input type="text" name="invois_no" class="form-control form-control-lg" v-model="currentUser.invois_no">
							</div>
							<div class="form-group">
	...
	data: {
		...
		showDeltModal: false,
		crecord: [],
		newUser: {Nama: "",invois_no: "",email: "",pNo: "",Alamat: "",Poskod: "",Bandar: "",Negeri: "",InstallTeam: "",installType: "",installDate: "",Remark: ""},
		currentUser: {},
	...
	methods: {
		getAllUsers(){
			axios.get("http://localhost/psmb/proses.php?action=read")
		...
		addUser(){
			...
			axios.post("http://localhost/psmb/proses.php?action=create", formData)
			.then(function(response){
				app.newUser = {Nama: "",invois_no: "",email: "",pNo: "",Alamat: "",Poskod: "",Bandar: "",Negeri: "",InstallTeam: "",installType: "",installDate: "",Remark: ""};
				if(response.data.error){
					app.errorMsg = response.data.message;
				}
				else{
		...
		updateUser(){
			...
			axios.post("http://localhost/psmb/proses.php?action=update", formData)
			.then(function(response){
				app.currentUser = {};
				if(response.data.error){
					app.errorMsg = response.data.message;
		...
		deleteUser(){
			var  formData = app.toFormData(app.currentUser);
			axios.post("http://localhost/psmb/proses.php?action=delete", formData)
			.then(function(response){
				app.currentUser = {};
				if(response.data.error){
					app.errorMsg = response.data.message;
